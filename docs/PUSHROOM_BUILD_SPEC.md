...
第 11 節的八個步驟全部完成並部署，另加三輪 UI/UX 演出與效能優化。
歷史紀錄見 [CHANGELOG.md](../CHANGELOG.md)，模組職責見 [README.md](../README.md)。

原始單檔版保留在 `docs/pushup-arena-mvp.html` 作為偵測演算法的對照組
（`tests/detect.test.mjs` 用它驗證重構後行為逐幀一致）。

**偵測邏輯已經過實機調校，不要重寫、不要「優化」演算法。**
第 3 節是它的規格，改動前必須先問人類。
`tests/detect.test.mjs` 就是用來擋住這件事的 —— 改了必跑。

### 尚未驗證
- 兩台實體裝置的多場對戰（規格第 11 節註明需要人類實測）
  → 實測腳本見 [PLAYTEST_2DEVICE.md](PLAYTEST_2DEVICE.md)，
    測試模式可開啟「對戰診斷」窗即時看房間狀態與時鐘偏移
- 長時間使用的熱節流表現
